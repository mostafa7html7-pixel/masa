...
const showUploadTooltip = () => {
    // إذا لم يره المستخدم من قبل
    if (!localStorage.getItem('upload_tooltip_seen')) {
        setTimeout(() => {
            uploadTooltip.classList.add('show');
        }, 1500); // إظهار بعد ثانية ونصف
    }
};

// استدعاء التلميح عند تحميل الصفحة
showUploadTooltip();
...
const handleUpload = async () => {
    ...
    try {
        ...
        // إخفاء التلميح وتسجيل أنه تمت رؤيته
        if (uploadTooltip.classList.contains('show')) {
            uploadTooltip.classList.remove('show');
            localStorage.setItem('upload_tooltip_seen', 'true');
        }
